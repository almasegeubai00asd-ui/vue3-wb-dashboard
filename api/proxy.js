...
// Базовый URL внешнего API — без /api в конце
const BASE_URL = 'http://109.73.206.144:6969'

// API-ключ
const API_KEY = process.env.VITE_API_KEY || 'E6kUTYrYwZq2tN4QEtyzsbEBk3ie'

// Любой GET-запрос к /api/* проксируем на внешний API
router.use(async (req, res) => {
  try {
    // req.path = /incomes, /orders и т.д.
    const url = `${BASE_URL}${req.path}`

    // Пробрасываем query-параметры + добавляем ключ
